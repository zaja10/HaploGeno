...
To identify genomic regions significantly associated with the phenotype, we use a Manhattan plot based on the variance of Local GEBVs. `HaploGeno` uses an Empirical Permutation Test to determine robust significance thresholds.
...
In addition to significance, it is often useful to visualize the magnitude of effect. The PVE plot shows the Marginal Phenotypic Variance Explained (Marginal PVE)
haplo$calculate_pve()

# Factor Analysis (FA-LMM)
# Extracts 3 latent factors from top 200 blocks using MLE on Covariance
haplo$analyze_block_structure(top_n = 200, factors = 3, use_covariance = TRUE)
...
# 7. Latent Factor Analysis (Haplo-FA)

For complex traits, multiple genomic blocks may vary together due to shared ancestry or selection. `HaploGeno` can perform internal Factor Analysis on the Local GEBVs to identify latent drivers of variance. This implementation uses a Maximum Likelihood (MLE) Factor Analysis on the block covariance matrix, consistent with FA-k models in biometric literature (FA-LMM).

```{r factor_heatmap, fig.width=6, fig.height=6}
# Visualize the genetic correlation structure implied by the top latent factors
haplo$plot_factor_heatmap()
```

This heatmap shows the correlations between high-variance blocks. Strong clusters indicate blocks that tend to be co-inherited or are structurally linked.
