...
In addition to significance, it is often useful to visualize the magnitude of effect. The PVE plot shows the Phenotypic Variance Explained by each block.

```{r manhattan_pve, fig.width=7, fig.height=5}
# Plot PVE of haplotype blocks
haplo$plot_manhattan(type = "pve")
```
...
For complex traits, multiple genomic blocks may vary together due to shared ancestry or selection. `HaploGeno` can perform internal Factor Analysis on the Local GEBVs to identify latent drivers of variance.
